...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('API Route (/api): Authorization header:', authorization ? 'Present' : 'Not present');
    ...
    console.log('API Route (/api): Checking for ROM at:', romPath);
    
    // If the file doesn't exist in the private directory, try the rom directory at the root level
    if (!fs.existsSync(romPath)) {
      console.log('API Route (/api): ROM not found in private directory, checking root rom directory');
    ...
    if (!fs.existsSync(romPath)) {
      console.error('API Route (/api): ROM file not found at:', romPath);
    ...
    console.log('API Route (/api): Serving ROM file from:', romPath);
    
    // Read the ROM file
    const romData = fs.readFileSync(romPath);
    console.log('API Route (/api): ROM file size:', romData.length, 'bytes');
    ...
    console.log('API Route (/api): Returning ROM file with headers:', Object.fromEntries(headers.entries()));
    ...
    console.error('API Route (/api): Error serving ROM file:', error);
    return NextResponse.json(
...
export async function OPTIONS(request: NextRequest) {
  console.log('API Route (/api): Received OPTIONS request for CORS preflight');
  ...
  console.log('API Route (/api): Returning OPTIONS response with headers:', Object.fromEntries(headers.entries()));
